...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bk-grid-bg" style={{ background: "#0A0F1E" }}>
      {/* Glow orbs */}
      ...
      <div className="w-full max-w-md px-4 animate-bk-fade-up">
        ...
        <div className="text-center mb-8">
          ...
          <p style={{ color: "#94A3B8", fontSize: "0.875rem" }}>AI-Powered Security Monitoring</p>
        </div>
        ...
        <div className="rounded-2xl p-8" style={{ background: "#0D1526", border: "1px solid rgba(0,212,255,0.15)", boxShadow: "0 0 40px rgba(0,212,255,0.05)" }}>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="font-bold" style={{ color: "#F1F5F9", fontSize: "1.1rem" }}>Welcome back</h2>
              <p style={{ color: "#94A3B8", fontSize: "0.75rem" }}>Sign in to your account</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Email */}
            <div>
              <label className="block mb-1.5" style={{ color: "#94A3B8", fontSize: "0.8rem", fontWeight: 500 }}>Email Address</label>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label style={{ color: "#94A3B8", fontSize: "0.8rem", fontWeight: 500 }}>Password</label>
                <Link href="/forgot-password" style={{ color: "#00D4FF", fontSize: "0.75rem", textDecoration: "none" }}>Forgot password?</Link>
              </div>
              <div className="relative">
                <Lock size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "#94A3B8" }} />
                <input
                  type={showPassword ? "text" : "password"}
                  className="bk-input pl-9 pr-10"
                  placeholder="••••••••"
                  value={form.password}
                  onChange={e => setForm(f => ({ ...f, password: e.target.value }))}
                />
                <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: "#94A3B8" }} onClick={() => setShowPassword(s => !s)}>
          ...
          <p className="text-center" style={{ color: "#94A3B8", fontSize: "0.8rem" }}>
            ...
            <Link href="/register" style={{ color: "#00D4FF", fontWeight: 500, textDecoration: "none" }}>Create account</Link>
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mt-4">
          ...
          <span style={{ color: "#94A3B8", fontSize: "0.7rem" }}>256-bit SSL encryption · Zero-trust architecture</span>
        </div>
      </div>
    </div>
  );
